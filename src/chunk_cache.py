...
def get_args():
    ...
    parser.add_argument("--dtype",                 type=str, default="bfloat16",
                        ...
                        help="Model dtype for the KV-cache prefill. bfloat16 matches "
                             "Qwen2.5's native precision (ROOT CAUSE 1); the stored "
                             "cache dtype string is taken from this so dequant restores "
                             "bf16 rather than re-corrupting via fp16.")
...
def main():
    ...
    if args.index_only:
        ...
        model = Qwen2ModifiedForCausalLM.from_pretrained(
            args.model_name,
            torch_dtype=model_dtype,      # ROOT CAUSE 1: bfloat16 for Qwen2.5
