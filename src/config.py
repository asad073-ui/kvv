...
def load_config(path: str | None = None, apply_mve: bool = True) -> SimpleNamespace:
    ...
    # Inject MODEL_SLUG so YAML paths can reference ${MODEL_SLUG}.
    # Users can pre-set MODEL_SLUG in the environment to override.
    model_name = (raw.get("model") or {}).get("name", "")
    os.environ.setdefault("MODEL_SLUG", _model_slug(model_name) if model_name else "unknown_model")

    # A4: guarantee SCRATCH_DIR resolves to a REAL path. When the shell did not
    # export it (e.g. evaluate.py invoked directly, not via scripts/*.sh), the old
    # behaviour left "${SCRATCH_DIR}" literal in every path, so results landed
    # under a directory literally named "${SCRATCH_DIR}". Default it to a concrete
    # project-local scratch dir so paths always expand to something real.
    here    = os.path.dirname(os.path.abspath(__file__))
    ...
    # A4 safety net: any ${VAR} that still survived expansion (an unset var that is
    # not SCRATCH_DIR/MODEL_SLUG) would silently create a literal directory. Surface
    # it loudly instead of writing results under a "${...}" path.
    ...
    if apply_mve and getattr(getattr(cfg, "mve", None), "enabled", False):
        ...
        # Fix 6: a dataset with num_examples <= 0 is disabled. This lets the
        # headline RGB-only run be expressed purely in the YAML (set nq_open and
        # hotpotqa num_examples to 0) without a CLI override.
        cfg.datasets_list = [
...
def config_to_chunk_cache_args(cfg: SimpleNamespace) -> list[str]:
    ...
    # Fix 2: standard_causal prefills at query time, so the offline per-chunk KV
    # caches are never read — build only the retrieval index to save disk.
